        ...
        if (IsCoinStake())
        {
                ...
                if (primeNodeRate) {
                    ...
                    if (nTime >= RESET_PRIMERATES && nTime < END_PRIME_PHASE_ONE) {
                        if (nStakeReward > GetProofOfStakeReward(nCoinAge, nTime, 100) - GetMinFee() + MIN_TX_FEE)
                            return DoS(100, error("ConnectInputs() : %s stake reward exceeded - Inside terms", GetHash().ToString().substr(0,10).c_str()));
                    } else {
                        if (nStakeReward > GetProofOfStakeReward(nCoinAge, nTime, primeNodeRate) - GetMinFee() + MIN_TX_FEE)
                            return DoS(100, error("ConnectInputs() : %s stake reward exceeded goto else,Stake Reward = %lld , an returned from fnc %lld", GetHash().ToString().substr(0,10).c_str(),nStakeReward,GetProofOfStakeReward(nCoinAge, nTime, primeNodeRate)));
                    }
                }
            }
            else
            {
                ...
                if(nStakeReward > GetProofOfStakeReward(nCoinAge, nTime, 0) - GetMinFee() + MIN_TX_FEE){
                    return DoS(100, error("ConnectInputs() : %s stake reward exceeded - OK Values are,Stake Reward = %lld , an returned from fnc %lld", GetHash().ToString().substr(0,10).c_str(),nStakeReward,GetProofOfStakeReward(nCoinAge, nTime, primeNodeRate)));
                }
            }
        }
        else
