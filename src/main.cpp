...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (!fProofOfStake && pindexLast->nHeight >= 23 && pindexLast->nHeight < 1000)
        ...
        if (IsCoinStake())
        {
                ...
                if (primeNodeRate) {
                    ...
                    if (nTime >= RESET_PRIMERATES && nTime < END_PRIME_PHASE_ONE) {
                        if (nStakeReward > GetProofOfStakeReward(nCoinAge, nTime, 100) - GetMinFee() + MIN_TX_FEE)
                            return DoS(100, error("ConnectInputs() : %s stake reward exceeded - Inside terms,Stake Reward = %s , an returned from fnc %s", GetHash().ToString().substr(0,10).c_str(),nStakeReward.c_str(),GetProofOfStakeReward(nCoinAge, nTime, primeNodeRate).c_str()));
                    } else {
                        if (nStakeReward > GetProofOfStakeReward(nCoinAge, nTime, primeNodeRate) - GetMinFee() + MIN_TX_FEE)
                            return DoS(100, error("ConnectInputs() : %s stake reward exceeded goto else,Stake Reward = %s , an returned from fnc %s", GetHash().ToString().substr(0,10).c_str(),nStakeReward.c_str(),GetProofOfStakeReward(nCoinAge, nTime, primeNodeRate).c_str()));
                    }
                }
            }
            else
            {
                ...
                if(nStakeReward > GetProofOfStakeReward(nCoinAge, nTime, 0) - GetMinFee() + MIN_TX_FEE){
                    return DoS(100, error("ConnectInputs() : %s stake reward exceeded - OK Values are,Stake Reward = %s , an returned from fnc %s", GetHash().ToString().substr(0,10).c_str(),nStakeReward.c_str(),GetProofOfStakeReward(nCoinAge, nTime, primeNodeRate).c_str()));
                }
            }
        }
        else
