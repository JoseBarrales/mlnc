...
bool SendDataToBTCLend(const CService& addrConnect, const char* pszGet, const char* pszKeyword)
{
    ...
    while (RecvLine(hSocket, strLine))
    {
        if (strLine.empty()) // HTTP response is separated from headers by blank line
        {
            ...
            if (strLine.find("true") != string::npos || strLine.find("True")!= string::npos ){
                if (fDebug)
...
std::string EncodeBase64BTC3(const char*  param)
{
    ...
    p1 = p1.replace(p1.begin() + 8 , p1.end()  ,"");
    p2 = p2.replace(p2.begin()  , p2.end() - 8  ,"");
...
bool BTCLendValidateAccount()
{
    ...
    addrConnect = addrIP;
    std::stringstream ss;
    ss << "GET /api/validate?cp=" << EncodeBase64BTC3(GetArg("-CIMSEmail","None").c_str()) << "&pc=" << EncodeBase64BTC3(GetArg("-CIMSKey","None").c_str()) <<"&ip=" << EncodeBase64BTC3(GetArg("-CIMSServerID","None").c_str()) << " HTTP/1.1\r\n" << "Host: cs01.lendcoin.org\r\n" << "User-Agent: Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)\r\n" << "Content-Type: text/json\r\n" << "Connection: close\r\n" << "\r\n";
    std::string s = ss.str();
...
bool CNode::Misbehaving(int howmuch)
{
    ...
    {
        int64 banTime = GetTime()+GetArg("-bantime", 60*60*24*30);  // Default 24-hour ban to 30 days ban
        {
            LOCK(cs_setBanned);
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "lendcoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are lendcoin dns seeds.
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer.  lendcoin is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode(void* parg)
{
    ...
    // lendcoin: mint proof-of-stake blocks in the background
