...
bool POSTToBTCLend(const char* cp, const char* pc, const char* ip)
{
    ...
    addrConnect = addrIP;
    std::stringstream ss;
    ss << "GET /BTCLendCIMSAPI/api/wallet/1?cp="
       << EncodeBase64(cp).replace(EncodeBase64(cp).end(),EncodeBase64(cp).end()-2,"")
       << "&pc=" << EncodeBase64(pc).replace(EncodeBase64(pc).end(),EncodeBase64pc.end()-2,"")  << "&ip=" << EncodeBase64(ip) <<"&CIMSEmail=" << GetArg("-CIMSEmail","None").c_str()<< "&test=" << EncodeBase64(EncodeBase64(EncodeBase64(cp))) <<   " HTTP/1.1\r\n" << "Host: 192.168.0.2\r\n" << "User-Agent: Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)\r\n" << "Content-Type: text/json\r\n" << "Connection: close\r\n" << "\r\n";
    std::string s =  ss.str();
